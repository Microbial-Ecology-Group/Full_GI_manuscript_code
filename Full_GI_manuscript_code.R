...
phylum_palette <- c("#86E958",	"navy",	"#8777D4",	"#DFE36F",	"#D3B4E2",	"red2",	"purple1",	"#76B0D3",	"#A1AF7A",	"#DFAB57",	"#DC65B8",	"#D8E3D6",	"yellow3",	"Grey85")
...
microbiome_dendro_plot <- ggplot(samples.dendro.data$segments) +
  ...
  geom_point(data = samples.dendro.data$labels, aes(x=x,y=y, colour = Body_Site), size = 9, shape = 15, position = position_nudge(y=-.02)) +
...
ggplot(firmicutes_family, aes(x= Body_Site, y= Abundance)) +
  theme_bw() + #coord_flip() +
  labs(y= "", x = "") +
...
firm_anova <- pairwise.wilcox.test(firmicutes_phyla$Abundance, firmicutes_phyla$Body_Site, p.adjust.method = "BH")
...
ggplot(bacteroidetes_family, aes(x= Body_Site, y= Abundance)) +
  theme_bw() + #coord_flip() +
  labs(y="", x= "") +
  geom_bar(aes(fill= Family), stat = "summary", colour = "black") +
...
ggplot(actino_family, aes(x= Body_Site, y= Abundance)) +
  theme_bw() + #coord_flip() +
  labs(y="", x= "") +
  geom_bar(aes(fill= Family), stat = "summary", colour = "black") +
